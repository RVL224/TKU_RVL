...
class MobileNetV2_fpn_r3(nn.Module):
    def __init__(self, width_mult=1.0, inverted_residual_setting=None):
        super(MobileNetV2_fpn_r3, self).__init__()
        block = InvertedResidual
        input_channel = 32
        last_channel = 1280
        
        self.reduce2 = nn.Conv2d(512, 256, kernel_size=1, stride=1, padding=0)
        self.reduce1 = nn.Conv2d(1280, 256, kernel_size=1, stride=1, padding=0)
        self.reduce0 = nn.Conv2d(96, 256, kernel_size=1, stride=1, padding=0)
...
@registry.BACKBONES.register('mobilenet_v2_fpn_r3')
def mobilenet_v2_fpn_r3(cfg, pretrained=True):
    model = MobileNetV2_fpn_r3()
    if pretrained:
