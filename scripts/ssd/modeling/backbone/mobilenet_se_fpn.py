...
class MobileNetV2_se_fpn(nn.Module):
    def __init__(self, width_mult=1.0, inverted_residual_setting=None):
        super(MobileNetV2_se_fpn, self).__init__()
...
@registry.BACKBONES.register('mobilenet_v2_se_fpn')
def mobilenet_v2_se_fpn(cfg, pretrained=True):
    model = MobileNetV2_se_fpn()
    if pretrained:
