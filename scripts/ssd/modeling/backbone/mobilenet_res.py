...
class MobileNetV2_res(nn.Module):
    def __init__(self, width_mult=1.0, inverted_residual_setting=None):
        super(MobileNetV2_res, self).__init__()
    ...
    def forward(self, x):
        ...
        for i in range(14):
            x = self.features[i](x)
        features.append(self.resblock[0](x))

        for i in range(14, len(self.features)):
            x = self.features[i](x)
        features.append(self.resblock[1](x))
    
        for i in range(len(self.extras)):
            x = self.extras[i](x)
            features.append(self.resblock[i+2](x))

        return tuple(features)


@registry.BACKBONES.register('mobilenet_v2_res')
def mobilenet_v2_res(cfg, pretrained=True):
    model = MobileNetV2_res()
    if pretrained:
