...
class MobileNetV2_7features(nn.Module):
    def __init__(self, width_mult=1.0, inverted_residual_setting=None):
        super(MobileNetV2_7features, self).__init__()
        ...
        self.extras = nn.ModuleList([
            ...
            InvertedResidual(256, 256, 2, 0.25)
        ])
    ...
    def forward(self, x):
        ...
        s0 = self.reduce0(features[0])
        s1 = self.reduce1(features[1])
        s2 = self.reduce2(features[2])
        s3 = self.reduce3(features[3])
        s4 = features[4]
        s5 = features[5]
        s6 = features[6]
...
@registry.BACKBONES.register('mobilenet_v2_7features')
def mobilenet_v2_7features(cfg, pretrained=False):
    model = MobileNetV2_7features()
    if pretrained:
