...
from lib.utility.nms_utils import cpu_nms, gpu_nms
from lib.utility.data_utils import parse_line
            ...
            for line in f:
                ...
                for i in range(len(labels)):
                    ...
                    if letterbox_resize:
                        ...
                        objects.append([int(x_min * resize_ratio + dw),
                                        int(y_min * resize_ratio + dh),
                                        int(x_max * resize_ratio + dw),
                                        int(y_max * resize_ratio + dh),
                                        label])
                    else:
                        # objects.append([int(x_min * new_width / ori_width),
                        #                 int(y_min * new_height / ori_height),
                        #                 int(x_max * new_width / ori_width),
                        #                 int(y_max * new_height / ori_height),
                        #                 label])
                        objects.append([int(x_min),
                                        int(y_min),
                                        int(x_max),
                                        int(y_max),
                                        label])
                gt_dict[img_id] = objects
                pic_dict[img_id] = pic_path
                # if count >= 2:
                #     break
                # count += 1
                # break
    return gt_dict, pic_dict
