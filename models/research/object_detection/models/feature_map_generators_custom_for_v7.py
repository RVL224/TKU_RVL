...
def cfe_feature_maps(image_features,
                     depth,
                     scope=None):

  with tf.name_scope(scope, 'lr'):
    ...
    with slim.arg_scope(
      ...
      for level in range(num_levels):
        ...
        # left (w > h)
        left_pad_w = width_padding(residual, kernel_size)  
        left = slim.separable_conv2d(
          left_pad_w,
          None, [kernel_size, 1],
          depth_multiplier=1,
          padding=padding,
          stride=1,
          scope='left_%d' % (level + 1) + "_depthwise_w")
             
        left_pad_h = height_padding(left, kernel_size)     
        left = slim.separable_conv2d(
          left_pad_h,
          None, [1, kernel_size],
          depth_multiplier=1,
          padding=padding,
          stride=1,
          scope='left_%d' % (level + 1) + "_depthwise_h")

        # right (h > w)
        right_pad_h = height_padding(residual, kernel_size)
        right = slim.separable_conv2d(
          right_pad_h,
          None, [1 ,kernel_size],
          depth_multiplier=1,
          padding=padding,
          stride=1,
          scope='right_%d' % (level + 1) + "_depthwise_h")
        
        right_pad_w = width_padding(right, kernel_size) 
        right = slim.separable_conv2d(
          right_pad_w,
          None, [kernel_size, 1],
          depth_multiplier=1,
          padding=padding,
          stride=1,
          scope='right_%d' % (level + 1) + "_depthwise_w")

        # smooth
        concat_net = tf.concat((left, right), 3)
        smooth = slim.conv2d(
          concat_net,
          depth, [1, 1],
          padding=padding,
