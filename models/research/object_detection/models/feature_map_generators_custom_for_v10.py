  ...
  with tf.name_scope(scope, 'top_down'):
    ...
    with slim.arg_scope(
      ...
      for level in reversed(range(num_levels - 1)):
        ...
        # ===========
        # custom 
        if(image_features[level][1].shape[1] > 4):
          residual = slim.conv2d(
        ...
        if use_explicit_padding:
          # top_down_ = top_down
          # top_down = ops.fixed_padding(top_down, kernel_size)
          top_down_pad = ops.fixed_padding(top_down, kernel_size)


        if use_depthwise:
          # from nets.mobilenet import conv_blocks
          # net = conv_blocks.expanded_conv(top_down,
          #                                 depth,
          ...
          dw_scope = 'smoothing_%d' % (level + 1) + "_depthwise"
          net = slim.separable_conv2d(
            # top_down,
            top_down_pad,
            None, [kernel_size, kernel_size],
            depth_multiplier=1,
            padding=padding,
            stride=1,
            scope=dw_scope)

          pw_scope = 'smoothing_%d' % (level + 1) + "_project"
          net = slim.conv2d(
              net,
              depth, [1, 1],
              padding=padding,
...
def rfb_feature_maps(image_features,
                     depth,
                     scope=None):

  with tf.name_scope(scope, 'basic'):
    num_levels = len(image_features)
    output_feature_maps_list = []
    output_feature_map_keys = []
    padding = 'VALID'
    ...
    for item in image_features:
      inter_channels.append(int(item[1].shape[3]) // 8)
    ...
    with slim.arg_scope(
      ...
      for level in range(num_levels):
        ...
        branch_1 = slim.conv2d(
          image_features[level][1],
          inter_channels[level],
          [1, 1],
          padding=padding,
          stride=1,
          activation_fn=tf.nn.relu,
          scope='branch1_%d_smooth' % (level + 1))

        branch_1 = height_padding(branch_1, kernel_size=3, rate=1)
        branch_1 = slim.conv2d(
          branch_1,
          int(inter_channels[level]//2)*3,
          [1, 3],
          padding=padding,
          stride=1,
          activation_fn=tf.nn.relu,
          scope='branch1_%d_h' % (level + 1))

        branch_1 = width_padding(branch_1, kernel_size=3, rate=1)
        branch_1 = slim.conv2d(
          branch_1,
          int(inter_channels[level]//2)*3,
          [3, 1],
          ...
          stride=1,
          activation_fn=tf.nn.relu,
          scope='branch1_%d_w' % (level + 1))

        branch_1 = ops.fixed_padding(branch_1, kernel_size=3, rate=3)
        branch_1 = slim.separable_conv2d(
          branch_1,
          None, [3, 3],
          depth_multiplier=1,
          padding=padding,
          ...
          rate=[3,3],
          activation_fn=None,
          scope='branch1_%d_depthwise' % (level + 1))
        ...
        branch_2 = slim.conv2d(
          ...
          stride=1,
          activation_fn=tf.nn.relu,
          scope='branch2_%d_smooth_1' % (level + 1))
        
        branch_2 = ops.fixed_padding(branch_2, kernel_size=3, rate=1)
        branch_2 = slim.conv2d(
          branch_2,
          int(inter_channels[level]//2)*3,
          [3, 3],
          ...
          stride=1,
          activation_fn=tf.nn.relu,
          scope='branch2_%d_smooth_2' % (level + 1))
        ...
        branch_2 = slim.conv2d(
          ...
          stride=1,
          activation_fn=tf.nn.relu,
          scope='branch2_%d_smooth_3' % (level + 1))
        
        branch_2 = ops.fixed_padding(branch_2, kernel_size=3, rate=5)
        branch_2 = slim.separable_conv2d(
          branch_2,
          None, [3, 3],
          depth_multiplier=1,
          padding=padding,
          ...
          rate=1,
          activation_fn=None,
          scope='branch2_%d_depthwise' % (level + 1))
        ...
        net = slim.conv2d(
          ...
          stride=1,
          activation_fn=None,
          scope='linear_%d' % (level + 1))
        
        net += image_features[level][1]
        output_feature_maps_list.append(net)
        output_feature_map_keys.append('rfb_%s' % image_features[level][0])

      return collections.OrderedDict(reversed(
          list(zip(output_feature_map_keys, output_feature_maps_list))))

def width_padding(inputs, kernel_size, rate=1):
...
def height_padding(inputs, kernel_size, rate=1):
  """Pads the input along the spatial dimensions independently of input size.
...
def multi_resolution_mixnet_feature_maps(feature_map_layout, depth_multiplier,
                                        min_depth, insert_1x1_conv, image_features,mix_num,
                                        pool_residual=False):
  ...
  for index, from_layer in enumerate(feature_map_layout['from_layer']):
    layer_depth = feature_map_layout['layer_depth'][index]
    conv_kernel_size = [3,5,7,9][:mix_num]
    if 'conv_kernel_size' in feature_map_layout:
      conv_kernel_size = feature_map_layout['conv_kernel_size'][index]
    if from_layer:
      feature_map = image_features[from_layer]
      ...
      pre_layer = feature_maps[-1]
      pre_layer_depth = pre_layer.get_shape().as_list()[3]
      pre_layer_depth_hidden_dim = pre_layer_depth // mix_num
      intermediate_layer = pre_layer
      if insert_1x1_conv:
        layer_name = '{}_1_Conv2d_{}_1x1_{}'.format(
            base_from_layer, index, pre_layer_depth_hidden_dim)
        intermediate_layer = slim.conv2d(
            pre_layer,
            pre_layer_depth_hidden_dim, [1, 1],
            padding='SAME',
            stride=1,
            scope=layer_name)
      stride = 2
      ...
      if use_depthwise:
        ...
        for kernel_size in conv_kernel_size:
          if use_explicit_padding:
            padding = 'VALID'
            intermediate_layer_pad = ops.fixed_padding(
                intermediate_layer, kernel_size)
          layer_name = '{}_2_Conv2d_{}_{}x{}_s2_{}'.format(
            base_from_layer, index, kernel_size, kernel_size,
            pre_layer_depth_hidden_dim)
          feature_map = slim.separable_conv2d(
              intermediate_layer_pad,
              None, [kernel_size, kernel_size],
              depth_multiplier=1,
              padding=padding,
              stride=stride,
              scope=layer_name + '_depthwise')
          mix_feature_maps.append(feature_map)
        
        feature_map = tf.concat(mix_feature_maps,axis=3)

        layer_name = '{}_2_Conv2d_{}_s2_{}'.format(
