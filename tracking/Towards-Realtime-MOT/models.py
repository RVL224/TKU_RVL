...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nC, nID, img_size, yolo_layer, cfg):
        ...
        self.nA = nA  # number of anchors (3) #k nA = 4
        self.nC = nC  # number of classes (80) #k nC = 1
    ...
    def forward(self, p_cat,  img_size, targets=None, classifier=None, test_emb=False):
        #k p_cat.shape => [batch_size, c, h, w]
        #k p => class and regression 6AxHxW (A=nA) so p_cat[:, :24, ...]
        #k p_emb => embedding map DxHxW so p_cat[:, 24:, ...]
        ...
        p_emb = p_emb.permute(0,2,3,1).contiguous() #k DxHxW
        p_box = p[..., :4] #k 4AxHxW
        p_conf = p[..., 4:6].permute(0, 4, 1, 2, 3)  # Conf #k 2AxHxW
        ...
        if targets is not None:
            ...
            #print(tconf.shape, tbox.shape, tids.shape)
            ...
            nP = torch.ones_like(mask).sum().float() #返回維度相同值為1的矩陣
            if nM > 0:
