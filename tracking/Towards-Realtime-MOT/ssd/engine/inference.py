...
from ssd.data.build import make_data_loader, make_train_eval_data_loader
...
def train_inference(model, data_loader, dataset_name, device, output_folder=None, use_cached=False, **kwargs):
    dataset = data_loader.dataset
    logger = logging.getLogger("SSD.train_inference")
    logger.info("Evaluating {} train_dataset({} images):".format('total_training_datasets', len(dataset)))
    predictions_path = os.path.join(output_folder, 'predictions.pth')
    if use_cached and os.path.exists(predictions_path):
        predictions = torch.load(predictions_path, map_location='cpu')
    else:
...
@torch.no_grad()
def do_train_evaluation(cfg, model, distributed, **kwargs):
    if isinstance(model, torch.nn.parallel.DistributedDataParallel):
        model = model.module
    model.eval()
    device = torch.device(cfg.MODEL.DEVICE)
    data_loaders_val = make_train_eval_data_loader(cfg, is_train=True, distributed=distributed)
    train_results = []
    for dataset_name, data_loader in zip(cfg.DATASETS.TRAIN, data_loaders_val):
        output_folder = os.path.join(cfg.OUTPUT_DIR, "train_inference", dataset_name)
    if not os.path.exists(output_folder):
        mkdir(output_folder)
    train_result = train_inference(model, data_loader, dataset_name, device, output_folder, **kwargs)
