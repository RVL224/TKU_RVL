...
class SSDBoxPredictor(BoxPredictor):
    ...
    def id_block(self, level, out_channels, boxes_per_location):
        return nn.Conv2d(out_channels, self.cfg.MODEL.EMB_DIM, kernel_size=3, stride=1, padding=1)
...
class SSDLiteBoxPredictor(BoxPredictor):
    ...
    def id_block(self, level, out_channels, boxes_per_location):
        num_levels = len(self.cfg.MODEL.BACKBONE.OUT_CHANNELS)
        if level == num_levels - 1:
            return nn.Conv2d(out_channels, self.cfg.MODEL.EMB_DIM, kernel_size=1)
        return SeparableConv2d(out_channels, self.cfg.MODEL.EMB_DIM, kernel_size=3, stride=1, padding=1)
