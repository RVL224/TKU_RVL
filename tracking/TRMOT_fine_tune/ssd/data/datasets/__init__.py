...
def build_train_eavl_dataset(dataset_list, transform=None, target_transform=None, is_train=True):
    assert len(dataset_list) > 0
    datasets = []
    for dataset_name in dataset_list:
        data = DatasetCatalog.get(dataset_name)
        ...
        args['target_transform'] = target_transform
        if factory == VOCDataset:
            args['keep_difficult'] = is_train #為了跟test一樣格式,所以把not拔掉
        elif factory == COCODataset:
            args['remove_empty'] = not is_train #為了跟test一樣格式,所以新增not拔掉
        dataset = factory(**args)
        datasets.append(dataset)
    # for testing, return a list of datasets
    #底下的is_train造原來的是為了確保train_dataset有多個的時候會把dataset合併為一個
    if not is_train:
        return datasets
    dataset = datasets[0]
    if len(datasets) > 1:
