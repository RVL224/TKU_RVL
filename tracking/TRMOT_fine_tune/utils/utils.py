...
def build_targets_thres(target, anchor_wh, nA, nC, nGh, nGw):
    ...
    #print(anchor_wh, nA, nC, nGh, nGw) #k anchor_wh , 4, 1, 13 26 52, 13 26 52 
    #k target(list物件)的數量由batch_size決定
    #k 每筆資料裡面的第一個維度為物件的數量
    #k 第二個維度固定為6 (class id bbox)
    #print(target[0]) #k
    #print(nB, 'nB') #k
    #for b in range(nB): #k
    #    print(target[b].shape, '%s'%b*6) #k
    #print('='*20) #k
    ...
    for b in range(nB):
        t = target[b]
        t_id = t[:, 1].clone().long().cuda() #k ID t_id.shape -> (nTb)
        t = t[:,[0,2,3,4,5]] #k class, x, y, w, h
        nTb = len(t)  # number of targets
        if nTb == 0:
            continue
...
def generate_anchor(nGh, nGw, anchor_wh):
    ...
    #print(anchor_wh.shape) #k (4, 2)
    mesh = torch.stack([xx, yy], dim=0)                                              # Shape 2, nGh, nGw
    #k torch.repeat https://zhuanlan.zhihu.com/p/58109107
...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.4, method=-1):
    ...
    for image_i, pred in enumerate(prediction):
        ...
        v = pred[:, 4] > conf_thres
        v = v.nonzero().squeeze() # 返回非零值的索引，每一行都是一个非零值的索引值
        if len(v.shape) == 0:
            v = v.unsqueeze(0)
...
def load_trained_weight_to_model(train_model_dict, need_training_model_dict):
    for layer_name in train_model_dict.keys():
            need_training_model_dict[layer_name] = train_model_dict[layer_name]
    return need_training_model_dict
