...
from utils.datasets import LoadImagesAndLabels, ssd_JointDataset, test_ssd_collate_fn, ssd_collate_fn
...
def test(
    ...
    # Get dataloader
    #k transforms = T.Compose([T.ToTensor()])
    #k dataset = JointDataset(dataset_root, test_path, img_size, augment=False, transforms=transforms)
    #k dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=False, 
    #k                                         num_workers=8, drop_last=False, collate_fn=collate_fn) 
    dataset = ssd_JointDataset(cfg, dataset_root, test_path, img_size, is_train=False)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True,
                                             num_workers=8, pin_memory=True, drop_last=True, collate_fn=test_ssd_collate_fn) 
    ...
    for batch_i, (imgs, targets, paths, shapes, targets_len) in enumerate(dataloader):
        ...
        targets_labels = [targets['labels'][i][:int(l)] for i,l in enumerate(targets_len)]
        targets_ids = [targets['ids'][i][:int(l)] for i,l in enumerate(targets_len)]
        targets_boxes = [targets['boxes'][i][:int(l)] for i,l in enumerate(targets_len)]

        for si, (classes, ids, boxes, detections) in enumerate(zip(targets_labels, targets_ids, targets_boxes, output)):
            if len(classes.shape) == 1:
                classes = classes.unsqueeze(1)
                ids = ids.unsqueeze(1)
            labels = torch.cat([classes.float(), ids.float(), boxes], dim = 1)
            seen += 1
            ...
            if labels.size(0) == 0:
                ...
                for *pred_bbox, conf, obj_conf  in detections:
                    ...
                    if iou[best_i] > iou_thres and obj_pred == labels[best_i, 0] - 1 and best_i not in detected:
                        correct.append(1)
...
def test_emb(
    ...
    # Get dataloader 
    dataset = ssd_JointDataset(cfg, dataset_root, test_paths, img_size)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True,num_workers=8, pin_memory=True, drop_last=True, collate_fn=ssd_collate_fn)

    embedding, id_labels = [], []
    print('Extracting pedestrain features...')

    emb_scale = math.sqrt(2) * math.log(nID-1) #k

    for batch_i, (imgs, targets, paths, img_path, targets_len) in enumerate(dataloader):

        '''new_2020_0212_22:26
        if len(id_labels) > 300:
           break
        '''
        t = time.time()
        ...
        output = model(imgs.cuda(), targets, targets_len.cuda(), test_emb=True)
    ...
    for f,fa in enumerate(far_levels):
        with open('./%s/log.txt' %cfg.OUTPUT_DIR, 'a') as output_log:
            output_log.write('TPR@FAR={:.7f}: {:.4f}'.format(fa, tar_at_far[f]) + '\n')
